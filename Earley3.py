...
def predict(col, rule,f):
    f.write("Predict: {col}")
    f.write(str(col))
    f.write("\n")
    f.write("Predict: {rule}")
    f.write(str(rule))
    f.write("\n")
    f.write("Predict: Production")
    f.write(str(rule.productions))
    f.write("\n")
    for prod in rule.productions:
        col.add(State(rule.name, prod, 0, col))

def scan(col, state, token,f):
    f.write("Scan {token}")
    f.write(str(token))
    f.write("\n")
    f.write("Scan: {col}")
...
def complete(col, state,f):
    f.write("Complete: {col} ")
    f.write(str(col))
    f.write("\n")
    f.write("Complete: {state} ")
    f.write(str(state))
    f.write("\n")
    if not state.completed():
        return
    f.write("Complete: Term")
    for st in state.start_column:
        ...
        if term.name == state.name:
            f.write(str(term))
            f.write("\n")
            str1 = "Complete: "+str(st.name)+"->"+str(st.production)+" ["+str(st.dot_index)+" "+str(st.start_column)+"]"
            f.write(str1)
            col.add(State(st.name, st.production, st.dot_index + 1, st.start_column))

ROOT= u"ROOT"

def parse(rule, text,f):
    table = [Column(i, tok) for i, tok in enumerate([None] + text.lower().split())]
    table[0].add(State(ROOT, Production(rule), 0, table[0]))
    for i, col in enumerate(table):
        for state in col:
            if state.completed():
                complete(col, state,f)
                f.write("----------------------\n")
            else:
                ...
                if isinstance(term, Rule):
                    ...
                    scan(table[i+1], state, term,f)
                    f.write("-------------------------\n")



    for st in table[-1]:
        if st.name == ROOT and st.completed():
            return st
    else:
